...
mode=${1:-help}; [[ $# == 0 ]] || shift
work='' namespace=''
usage() { printf 'Usage: bash scripts/controller-build.sh build --work-dir NEW_EXTERNAL_DIRECTORY\n       bash scripts/controller-build.sh publish --work-dir DIRECTORY --confirm-namespace ghcr.io/orangeshyguy21/proofstorm\n'; }
while [[ $# -gt 0 ]]; do
  case "$1" in
    --work-dir|--confirm-namespace)
      [[ $# -ge 2 && -n "$2" ]] || { usage >&2; exit 2; }
      case "$1" in --work-dir) work=$2 ;; --confirm-namespace) namespace=$2 ;; esac
      shift 2 ;;
    *) usage >&2; exit 2 ;;
  esac
done
case "$mode" in
  ...
  build) [[ -n "$work" && -z "$namespace" ]] || { usage >&2; exit 2; } ;;
  publish) [[ -n "$work" && "$namespace" == ghcr.io/orangeshyguy21/proofstorm ]] || { usage >&2; exit 2; } ;;
  *) usage >&2; exit 2 ;;
esac
...
if [[ "$mode" == build ]]; then
  ...
  "$helper" release-controller prepare "$root" "$work" > "$plan_file"
  plan=()
  ...
  printf 'Building the matching Linux AMD64 controller and execution helper...\n'
  "$helper" release-run 3600 docker buildx build --platform linux/amd64 --provenance=false --load \
...
probe=(run --rm --platform linux/amd64 --network none --read-only --cap-drop ALL
