...
PROMPT="$PROMPT Absolute budget (Unix seconds): start=$STARTED_EPOCH, cleanup=$(( STARTED_EPOCH + MAX_SECONDS * 80 / 100 )), hard_stop=$(( STARTED_EPOCH + MAX_SECONDS )). Tool responses include _benchmark_budget with current time, phase and remaining seconds. On cleanup phase, immediately stop waiting for experimental completion and cancel remaining work; then finish teardown and reporting. Observation waits are shortened at the cleanup boundary and to at most 10 seconds during cleanup; execution deadlines are unchanged. Call lab_close, then lab_wait with target_phase=closed to verify absence. Do not wait for the hard cap to write your report."
