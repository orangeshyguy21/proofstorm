...
for golden in cdk cdk-cln-cell cdk-bdk cdk-lnd-bdk cdk-postgres-cell cdk-ldk cdk-grpc-processor; do
  if [[ "$(public_image "$golden")" != "$STANDARD_IMAGE" ]]; then
    echo "CDK rendering $golden does not use the CDK mint image" >&2
    exit 1
  fi
done
...
for golden in cdk cdk-cln-cell cdk-bdk cdk-lnd-bdk cdk-postgres-cell cdk-grpc-processor; do
