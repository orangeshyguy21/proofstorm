use crate::{freshness::UpdatedAgo, model::sat, relationships::msat};
use leptos::prelude::*;
...
pub fn RelationshipPanel(
    ...
    let holdings_stale = move || {
        holdings.get().is_some_and(|o| {
            o.error.is_some() || crate::freshness::now().saturating_sub(o.observed_at_unix) > 20
        })
    };
    ...
    view! {
        <Show when=move ||holdings.get().is_some()>
            <div class="holdings-panel"><h4>"Holdings by mint"</h4>
                <Show when=holdings_stale><p class="observation-warning">{move ||if holdings.get().is_some_and(|o|o.observed_at_unix>0) {"Stale · showing last observed holdings"}else{"Holdings unavailable"}}</p></Show>
                ...
                <small>{move ||holdings.get().map(|o|view!{<UpdatedAgo unix=o.observed_at_unix label="Observed" />})}</small>
        ...
        <Show when=move ||channels.get().is_some()>
            <div class="channels-panel"><h4>"Open channels"</h4>
                <Show when=channels_stale><p class="observation-warning">{move ||if channels.get().is_some_and(|o|o.observed_at_unix>0) {"Stale · showing last observed channels"}else{"Channels unavailable"}}</p></Show>
                ...
                <small>{move ||channels.get().map(|o|view!{<UpdatedAgo unix=o.observed_at_unix label="Observed" />})}</small>
