        ...
        <Show when=move || lab.get().is_some()>
            <section class="lab-workspace">
                <div class="canvas-heading">
                    {move || lab.get().map(|lab| view! {<h1 title=lab.id.clone()>{lab_name(&lab)}</h1><span class="phase-badge">{lab_phase(&lab)}</span><span class="lab-count">{format!("{} components · {} embedded",lab.components.items.len(),lab.components.items.iter().filter_map(|c|c.details.as_ref()).map(|d|d.embedded.len()).sum::<usize>())}</span>})}
                ...
                <div class="canvas-layout">
                    ...
                    <Show when=move || !selected_component.get().is_empty()>
                        <aside class="inspector" aria-label="Component details">
                            ...
                            <crate::relationship_panel::RelationshipPanel telemetry lab selected=selected_component />
                            {move || lab.get().and_then(|lab|lab.components.items.iter().find(|c|c.id==selected_component.get()).cloned().map(|c|(lab,c))).map(|(lab,component)|view!{<ComponentDiagnostics component lab />})}
                        </aside>
                    </Show>
                </div>
