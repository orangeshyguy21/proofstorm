#![cfg(feature = "runtime")]
use proofstorm_driver::management::{Empty, MintInfo, inspect, nutshell_info};
use rcgen::{
...
impl tonic::server::NamedService for Mint {
    const NAME: &'static str = "cashu.Mint";
}
impl tonic::server::UnaryService<Empty> for Mint {
    type Response = MintInfo;
    type Future = BoxFuture<Response<MintInfo>, Status>;
    fn call(&mut self, request: Request<Empty>) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            Ok(Response::new(MintInfo {
                name: Some("fixture mint".into()),
                version: Some("0.20.3".into()),
                description: Some(if mode == "oversize" {
                    "x".repeat(131_072)
                } else {
                    "fixture".into()
                }),
    ...
    B::Error: Into<StdError> + Send + 'static,
{
    ...
    fn call(&mut self, request: http::Request<B>) -> Self::Future {
        assert_eq!(request.uri().path(), "/cashu.Mint/GetInfo");
        let service = self.clone();
        Box::pin(async move {
            let codec = tonic_prost::ProstCodec::<MintInfo, Empty>::default();
...
#[tokio::test]
async fn management_authenticates_both_peers_and_rejects_untrusted_or_wrong_usage_certificates() {
    let credentials = Credentials::new("127.0.0.1");
    let server = Running::new(&credentials, "ok").await;
    let path = credentials.directory.path();
    let info = nutshell_info(&server.address, path).await.unwrap();
    assert_eq!(info.name.as_deref(), Some("fixture mint"));
    for identity in ["missing", "server"] {
        assert!(inspect(&server.address, path, identity).await.is_err());
    }
    assert!(
        inspect(
    ...
    // Trusted server, but a client signed by another CA.
    fs::copy(
        path.join("ca.pem"),
    ...
    .unwrap();
    assert!(
        nutshell_info(&server.address, unrelated.directory.path())
            .await
            .is_err()
    );
    // Correct client, but an untrusted server issuer.
    let untrusted = Credentials::new("127.0.0.1");
    fs::copy(
        untrusted.directory.path().join("ca.pem"),
        path.join("ca.pem"),
    )
    .unwrap();
    assert!(nutshell_info(&server.address, path).await.is_err());
    assert_eq!(server.calls.load(Ordering::SeqCst), 1);
    server.finish().await;
}

#[tokio::test]
async fn management_checks_the_server_name() {
    let credentials = Credentials::new("wrong.example");
    let server = Running::new(&credentials, "ok").await;
    assert!(
        nutshell_info(&server.address, credentials.directory.path())
            .await
            .is_err()
    );
...
#[tokio::test]
async fn management_rejects_failed_and_oversized_rpcs_and_bounds_a_stalled_rpc() {
    let credentials = Credentials::new("127.0.0.1");
    for mode in ["reject", "oversize", "stall"] {
