use proofstorm_core::{Capability, LabSpec, OperationKind, PublishedRevision};
...
fn purge_releases_name_history_and_retries_but_preserves_shared_data() {
    ...
    seed(&store);
    let revision = publish(&store, "first", &lab(&["chain"]));
    let old = store
    ...
    store
        ...
        .unwrap();
    publish(&store, "template", &lab(&["independent"]));
    let guard = store.try_lifecycle_guard().unwrap().unwrap();
    store.purge_lab(&old).unwrap();
    ...
    assert!(
        store
            .materialize("w", "agent", "demo", &revision.digest, "start")
            .is_err(),
        "old low-level retry must not resurrect a deleted lab"
    );
    let next = publish(&store, "fresh", &lab(&["different"]));
    let fresh = store
        .materialize("w", "agent", "demo", &next.digest, "new-start")
        .unwrap();
    assert_ne!(fresh.instance_key, old.instance_key);
    assert!(
        store.purge_lab(&old).is_err(),
...
fn a_fresh_plan_can_recreate_the_same_configuration_with_shared_revision() {
    ...
    seed(&store);
    let revision = publish(&store, "original-plan", &lab(&["chain"]));
    let old = store
    ...
    let shared = store
        ...
        .unwrap();
    store.purge_lab(&old).unwrap();
    let fresh_revision = publish(&store, "new-plan", &lab(&["chain"]));
