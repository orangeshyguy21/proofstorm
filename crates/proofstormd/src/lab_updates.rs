//! Adopt old inventories and reconcile resources removed from a live lab.
use super::{
    Api, BTreeMap, BTreeSet, ConfigMap, Context, DeleteParams, Error, ListParams, ProofstormLab,
    ProofstormLabAction, ResourceExt, instance_namespace,
};
use kube::api::{ApiResource, DynamicObject, GroupVersionKind, Preconditions};

pub async fn action_lab(
    action: &ProofstormLabAction,
    mut lab: ProofstormLab,
    context: &Context,
) -> Result<ProofstormLab, Error> {
    ...
    if revision == &lab.spec.revision_digest {
        return Ok(lab);
    }
    let digest = proofstorm_core::digest_json(&(&lab.spec.instance_key, revision));
    ...
    if let Some(spec) = map.data.as_ref().and_then(|d| d.get("spec.json")) {
        if let Ok(spec) = serde_json::from_str::<proofstorm_kube::ProofstormLabSpec>(spec) {
            if spec.instance_key == lab.spec.instance_key && spec.revision_digest == *revision {
                lab.spec = spec;
                if let Some(status) = lab.status.as_mut() {
                    for component in &mut status.components {
                        if lab.spec.lock.entries.iter().any(|e| {
    ...
    if lab.spec.revision_digest != *revision {
...
> {
    ...
    let desired = lab
        ...
        .map(|c| c.id.as_str())
        .collect::<BTreeSet<_>>();
    let delete_data: Vec<String> = lab
        .annotations()
    ...
    for (group, version, kind) in [
            ...
            .items
        {
            ...
            if !owned_resource(lab, &namespace, kind, component, &resource.name_any()) {
                continue;
            }
