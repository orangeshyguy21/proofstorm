//! Transport-independent admission and recovery of reviewed cell changes.
use super::Cells;
use crate::{Error, ErrorKind};
use proofstorm_core::{CellInstance, CellInstanceStatus, InstancePhase};
use proofstorm_core::{CellUpdatePlan, PublishedRevision, digest_json};
...
impl Cells {
    pub async fn materialize_revision(
        ...
        self.authorize(&[
            proofstorm_core::Capability::CellMaterialize,
    ...
    pub(super) async fn apply_draft_locked(
        ...
        if digest_json(&draft.cell) != plan_digest {
            ...
            Err(error) => {
                ...
                let detail = ReconciliationError {
                    code: error.details.as_ref().and_then(|d| d["code"].as_str())
                        .unwrap_or("cell_update_runtime").into(),
...
pub fn review_apply(
    ...
    let actual = if let Some(plan) = &update {
        ...
        let target = store.resolve_cell(workspace, principal, reference)?;
        ...
        digest_json(&store.read_draft(workspace, principal, plan_id)?.cell)
    };
    if actual != expected_digest {
        return Err(Error {
            kind: ErrorKind::Invalid,
            message: "stored cell plan does not match expected_plan_digest; nothing was applied"
                .into(),
            details: Some(
                serde_json::json!({"code":"cell_plan_digest_mismatch", "plan_id":plan_id,
