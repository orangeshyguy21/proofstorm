//! Shared cell application service. CLI and MCP adapt inputs and responses here;
//! runtime resources and durable storage remain behind their respective modules.
use crate::{Error, Runtime};
use proofstorm_core::{Capability, CellInstance, CellInstanceStatus, Experiment};
use proofstorm_store::{CellHandle, Store, StoreError};
...
impl Cells {
            ...
            crate::bootstrap::prepare_images(installation.clone(), revision.lock.clone())
                ...
                .map_err(|error| {
                    Error::problem(
                        ...
                        format!(
                            "{error:#}. Retry the same cell request; no resources were deleted."
                        ),
                    )
                })?;
        }
    ...
    fn instance(&self, cell: &CellHandle) -> Result<CellInstance, Error> {
        Ok(self
            .store
            .instance(&self.workspace, &self.principal, &cell.instance_id)?)
    }
}
...
mod image_tests {
    ...
    async fn denied_materialization_and_foreign_runtime_never_prepare_images() {
        ...
        let mut cells = Cells::new(store.clone(), runtime, "local".into(), "agent".into())
            .with_installation(Some(installation.clone()));
        let cell =
            serde_json::from_str(include_str!("../../../../examples/developer-cell.json")).unwrap();
        let lock =
            proofstorm_core::resolve_lock(&cell, proofstorm_core::default_catalog()).unwrap();
        ...
        store
            .revoke("local", "agent", Capability::CellMaterialize)
            .unwrap();
        cells.runtime.cluster_source = installation.context();
        assert!(cells.prepare_images(&revision).await.is_err());
