...
use proofstorm_core::{Capability, CellInstance, CellInstanceStatus, InstancePhase};
...
fn blocked(status: &CellInstanceStatus) -> bool {
...
impl Cells {
    fn wait_identity(&self, request: &WaitRequest<'_>) -> Result<CellInstance, Error> {
            ...
            Ok(instance) => {
                    ...
                    .is_some_and(|key| key != instance.instance_key)
                {
                    return Err(Error::problem(
                        "stale_incarnation",
                        "The cell was replaced; read its current status",
                    ));
                }
                Ok(instance)
            }
    ...
    pub async fn wait(&self, request: WaitRequest<'_>) -> Result<WaitResult, Error> {
        self.authorize(&[Capability::CellStatus])?;
        ...
        loop {
            let status = match tokio::time::timeout_at(deadline, self.observe_tracked(&tracked, request.target_phase)).await {
                ...
                Err(_) => return last.map_or_else(
                    || Err(Error::problem("cell_wait_deadline_exceeded", "the runtime status backend did not answer before the requested cell wait deadline")),
