...
pub(super) fn detect(harness: Harness, project: &Path) -> Result<LaunchPlan> {
        ...
        cfg!(target_os = "macos"),
        "native agent buttons currently support macOS; use --cli in a terminal"
    );
    ...
    Err(last_error.unwrap_or_else(|| anyhow::anyhow!("{} desktop was not found in Applications; install the native app, or use --cli in a terminal. Nothing was attached.", harness.name())))
}
